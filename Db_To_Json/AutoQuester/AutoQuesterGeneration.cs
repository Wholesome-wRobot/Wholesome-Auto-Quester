...
namespace Db_To_Json.AutoQuester
{
    internal class AutoQuesterGeneration
    {
        ...
        public static void Generate(SQLiteConnection con, SQLiteCommand cmd)
        {
            ...
            // The start quest can only be indicated in item itself
            List<AQModelItemTemplate> allItemsStartingAQuest = GetAllItemsStartingAQuest(con);
            foreach (AQModelItemTemplate itemStartingQuest in allItemsStartingAQuest)
            {
                AQModelQuestTemplate correspondingQuest = quests.Find(quest => quest.Id == itemStartingQuest.startquest);
                if (correspondingQuest != null)
                {
                    correspondingQuest.StartItem = itemStartingQuest.Entry;
                }
            }
        ...
        private static void CreateIndices(SQLiteCommand cmd)
        {
            ...
            ExecuteQuery(cmd, $@"
                ...
                CREATE INDEX IF NOT EXISTS `idx_item_template_startquest` ON `item_template` (`startquest`);
        ...
        private static void ExecuteQuery(SQLiteCommand cmd, string query)
        {
